...
/**
 * A service for CryptoRescueCore to enable HTTP routes to query information about the blockchain.
 *
...
var InsightAPI = function(options) {
  ...
  this.coinShort = options.coinShort || 'CPR';
  this.coinTicker = options.coinTicker || 'https://api.coinmarketcap.com/v1/ticker/cryptorescue/?convert=USD';
};

InsightAPI.dependencies = ['cryptorescued', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  this.node.services.cryptorescued.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.cryptorescued.on('block', this.blockEventHandler.bind(this));
