...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.cryptorescued.height - transaction.height + 1;
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var address = cryptorescuecore.Address(output.address); //TODO return type from cryptorescuecore-node
