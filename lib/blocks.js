...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.cryptorescued.height - blockCached.height + 1;
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.cryptorescued.getBlockHeader(hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix cryptorescuecore to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.cryptorescued.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.cryptorescued.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new cryptorescuecore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in cryptorescuecore-node.
      var header = cryptorescuecore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [cryptorescuecore.Transaction().fromBufferReader(br)];

      self.node.services.cryptorescued.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.cryptorescued.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.cryptorescued.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
