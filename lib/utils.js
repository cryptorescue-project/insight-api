...
UtilsController.prototype.estimateFee = function(req, res) {
  ...
  async.map(nbBlocks, function(n, next) {
    var num = parseInt(n);
    // Insight and CryptoRescue JSON-RPC return cryptorescue for this value (instead of satoshis).
    self.node.services.cryptorescued.estimateFee(num, function(err, fee) {
      if (err) {
        return next(err);
      }
      next(null, [num, fee]);
...
UtilsController.prototype.estimateSmartFee = function(req, res) {
  ...
  async.map(nbBlocks, function(n, next) {
    var num = parseInt(n);
    // Insight and CryptoRescue JSON-RPC return cryptorescue for this value (instead of satoshis).
    self.node.services.cryptorescued.estimateSmartFee(num, conservative, function(err, fee) {
      if (err) {
        return next(err);
      }
      next(null, [num, fee]);
